...
def discover_prospectus_detail_id() -> int:
    ...
    print("Prospectus discovery failed; using verified detail ID", PROSPECTUS_DETAIL_ID)
    print(" | ".join(errors))
    return PROSPECTUS_DETAIL_ID


def main() -> None:
    prospectus_id = discover_prospectus_detail_id()
    if prospectus_id != PROSPECTUS_DETAIL_ID:
        raise RuntimeError(
            f"Unexpected formal prospectus detail ID: {prospectus_id} != {PROSPECTUS_DETAIL_ID}"
        )
    ...
    replacements = [
        ("hehe_information_verified_package", "actions_technology_verified_package"),
        ...
        ("if years != [2024, 2025]:", "if years != [2021, 2022, 2023, 2024, 2025]:"),
        (
            "for 2024 and 2025, found {years}.",
            "for 2021 through 2025, found {years}.",
        ),
        (
        ...
        (
            '"year": 2024,\n        "label": "炬芯科技：首次公开发行股票并在科创板上市招股说明书"',
            '"year": 2021,\n        "label": "炬芯科技：首次公开发行股票并在科创板上市招股说明书"',
        ),
        (
            "if [item[\"year\"] for item in documents if item[\"category\"] == \"Annual Report\"] != [2024, 2025]:",
            "if [item[\"year\"] for item in documents if item[\"category\"] == \"Annual Report\"] != [2021, 2022, 2023, 2024, 2025]:",
        ),
        (
            "公司于2024年9月在上海证券交易所科创板上市；上市后独立年度报告为2024年和2025年，共2份。",
            "公司于2021年11月29日在上海证券交易所科创板上市；收录上市后全部独立年度报告2021—2025年，共5份。",
        ),
        (
            "招股说明书采用2024年9月23日披露的正式发行版，排除招股意向书、申报稿、注册稿和问询回复。",
            "招股说明书采用2021年11月24日披露的正式发行版，排除招股意向书、申报稿、注册稿和问询回复。",
        ),
        (
            "最新半年报为2026年8月20日披露、覆盖截至2026年6月30日止六个月的完整报告。",
            "最新半年报为2026年8月25日披露、覆盖截至2026年6月30日止六个月的完整报告。",
        ),
        ("if len(documents) != 4:", "if len(documents) != 7:"),
        ("Expected 4 documents", "Expected 7 documents"),
        ("if len(pdfs) != 4:", "if len(pdfs) != 7:"),
        ("Expected 4 PDFs", "Expected 7 PDFs"),
...
'''
    direct_encoded = (
        ...
        f"MRGG/CNSESH_STOCK/2021/2021-11/{PROSPECTUS_DATE}/{prospectus_id}.PDF"
    )
    direct_plain = (
...
prospectus_context = (
    "新浪财经公告页面列示：{PROSPECTUS_DATE} 炬芯科技首次公开发行股票并在科创板上市招股说明书；"
...
'''
    ...
    required = [
        ...
        f'LATEST_HALF_PUBLISHED_DATE = "{LATEST_HALF_DATE}"',
        f"{prospectus_id}.PDF",
        "炬芯科技_全部年报_招股说明书_2026年最新半年报_完整PDF.zip",
        "if len(documents) != 7:",
        "if len(pdfs) != 7:",
    ]
    ...
    compiled = compile(code, "actions_technology_generated_packaging.py", "exec")
