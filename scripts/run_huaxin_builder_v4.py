...
SOURCE = Path('scripts/build_huaxin_filings_package.py')
TEMP = Path('scripts/_build_huaxin_filings_package_v4_runtime.py')

text = SOURCE.read_text(encoding='utf-8')

# Small generated index-record PDFs are legitimate one-page PDFs.
text = text.replace("path.stat().st_size < 10_000", "path.stat().st_size < 500", 1)

# The official 1993 prospectus/listing HTML endpoints are indexed by CNINFO but
# currently return 404. Keep the accessible 1993 annual-summary HTML only.
...
# Insert transparent archival index records for the two unavailable 1993 files.
...
exec(compile(text, str(TEMP), 'exec'), {'__name__': '__main__', '__file__': str(TEMP)})
